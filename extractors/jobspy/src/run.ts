...
function resolveJobSpyTermConcurrency(override?: number): number {
  return parsePositiveIntInRange(
    override ?? process.env.JOBSPY_TERM_CONCURRENCY ?? 2,
    2,
    1,
    4,
  );
...
async function runWithConcurrency<T, TResult>(args: {
  items: T[];
  concurrency: number;
  task: (item: T, index: number) => Promise<TResult>;
}): Promise<TResult[]> {
  const { items, task } = args;
  ...
  const safeConcurrency = Math.max(
    ...
    Math.min(
      4,
      Math.floor(Number.isFinite(args.concurrency) ? args.concurrency : 1),
    ),
  );
...
export async function runJobSpy(
  ...
  try {
    ...
    const termConcurrency = resolveJobSpyTermConcurrency(
      options.termConcurrency,
    );
    const tasks: JobSpyTask[] = [];
    let runIndex = 0;

    for (const searchTerm of searchTerms) {
      for (const location of runLocations) {
        runIndex += 1;
        tasks.push({ searchTerm, location, runIndex, totalRuns });
      }
    }

    const taskResults = await runWithConcurrency({
      items: tasks,
      concurrency: termConcurrency,
      task: async (taskItem) => {
        const locationToken = taskItem.location ?? countryIndeed ?? "anywhere";
        const suffix = `${taskItem.runIndex}_${slugForFilename(taskItem.searchTerm)}_${slugForFilename(locationToken)}`;
        ...
        await new Promise<void>((resolve, reject) => {
          ...
          const child = spawn(pythonPath, [JOBSPY_SCRIPT], {
            ...
            env: {
              ...
              JOBSPY_SEARCH_TERM: taskItem.searchTerm,
              JOBSPY_TERM_INDEX: String(taskItem.runIndex),
              JOBSPY_TERM_TOTAL: String(taskItem.totalRuns),
              ...
              ...(taskItem.location
                ? { JOBSPY_LOCATION: taskItem.location }
                : {}),
    ...
    for (const taskJobs of taskResults) {
