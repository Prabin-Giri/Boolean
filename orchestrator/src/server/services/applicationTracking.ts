...
function getScopedJobWhere(applicationId: string) {
  const userId = getUserId();
  return and(
    eq(jobs.id, applicationId),
    ...(userId ? [eq(jobs.userId, userId)] : []),
  );
}
...
export async function getStageEvents(
  ...
  const job = await db
    .select({ id: jobs.id })
    .from(jobs)
    .where(getScopedJobWhere(applicationId))
    .limit(1);
...
export function updateStageEvent(
  ...
  db.transaction((tx) => {
    ...
    const job = tx
      .select({ id: jobs.id })
      .from(jobs)
      .where(getScopedJobWhere(event.applicationId))
      .get();
