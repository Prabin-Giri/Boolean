...
describe("Model Selection Logic", () => {
  ...
  describe("Tailoring Service", () => {
    ...
    it("falls back to the instance API key for signed-in users", async () => {
      currentUserId = "user-1";
      process.env.LLM_API_KEY = "instance-key";

      const runtime = await resolveLlmRuntimeSettings("tailoring");

      expect(runtime.apiKey).toBe("instance-key");
    });
  });
