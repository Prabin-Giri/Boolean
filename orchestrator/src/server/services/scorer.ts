...
interface SuitabilityModelOutput {
  score: number;
  reason: string;
  ...
  schema: {
    ...
    properties: {
      ...
      reason: {
        type: "string",
        description:
          "Specific fit summary referencing concrete skills, experience, or domain requirements from the role.",
      },
      matchedEvidence: {
        type: "array",
        description:
          "Specific evidence points that explain why the candidate matches this role.",
        items: {
          type: "string",
        },
      },
      missingEvidence: {
        type: "array",
        description:
          "Specific requirement gaps or unknowns that lower confidence.",
        items: {
          type: "string",
        },
      },
...
function normalizeEvidenceItems(items: unknown): string[] {
...
function buildKeywordOverlap(
  ...
  const matched = TECH_KEYWORDS.filter(
    (keyword) => jobText.includes(keyword) && profileText.includes(keyword),
  ).slice(0, 4);
  const missing = TECH_KEYWORDS.filter(
    (keyword) => jobText.includes(keyword) && !profileText.includes(keyword),
  ).slice(0, 2);
...
function isGenericReason(reason: string): boolean {
  ...
  return (
    normalized.includes("good match") ||
    normalized.includes("strong match") ||
    normalized.includes("decent fit") ||
    normalized.includes("suitable role")
  );
}

function enrichSuitabilityReason(
  output: SuitabilityModelOutput,
  job: Job,
  profile: Record<string, unknown>,
): string {
  const base = (output.reason || "").trim() || "No explanation provided";
  const matchedEvidence = normalizeEvidenceItems(output.matchedEvidence);
  const missingEvidence = normalizeEvidenceItems(output.missingEvidence);

  if (matchedEvidence.length > 0 || missingEvidence.length > 0) {
    return [
      base,
      matchedEvidence.length > 0
        ? `Matched evidence: ${matchedEvidence.join("; ")}`
        : null,
      missingEvidence.length > 0
        ? `Potential gaps: ${missingEvidence.join("; ")}`
        : null,
    ]
  ...
  const { matched, missing } = buildKeywordOverlap(job, profile);
  const detailLines = [
    matched.length > 0 ? `Matched requirements: ${matched.join(", ")}` : null,
    missing.length > 0 ? `Potential gaps: ${missing.join(", ")}` : null,
  ].filter((line): line is string => Boolean(line));

  if (detailLines.length === 0) {
    return isGenericReason(base)
      ? `${base}\nRole context: ${job.title} at ${job.employer}.`
      : base;
  }
...
export async function scoreJobSuitability(
  ...
  const result = await llm.callJson<SuitabilityModelOutput>({
    model,
  ...
  const clampedReason = enrichSuitabilityReason(result.data, job, profile);
