...
describe.sequential("envSettings overrides", () => {
  ...
  it("inherits env-backed LLM defaults for signed-in users without overrides", async () => {
    process.env.LLM_PROVIDER = "openai";
    const { runWithRequestContext } = await import("../infra/request-context");
    const { getEnvSettingsData } = await import("./envSettings");

    const values = await runWithRequestContext({ userId: "user-1" }, () =>
      getEnvSettingsData(),
    );

    expect(values.llmApiKeyHint).toBe("sk-e");
  });
});
