...
export async function getCurrentDesignResume(): Promise<DesignResumeDocument | null> {
  try {
    return hydrateDocument(await designResumeRepo.getLatestDesignResumeDocument());
  } catch (error) {
...
export async function getDesignResumeStatus(): Promise<DesignResumeStatusResponse> {
  try {
    ...
    return {
      ...
      documentId: document ? getCurrentDesignResumeDocumentId() : null,
...
function getCurrentDesignResumeDocumentId(): string {
  const userId = getUserId();
  return userId ? `user:${userId}:${DESIGN_RESUME_DEFAULT_ID}` : DESIGN_RESUME_DEFAULT_ID;
}
