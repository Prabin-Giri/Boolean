...
async function readV5Credentials(overrides?: ResolveModeOptions["v5"]) {
  const hasScopedUser = Boolean(getUserId());
  ...
  const apiKey = resolveOverrideValue({
    ...
    envValue: hasScopedUser ? undefined : process.env.RXRESUME_API_KEY,
  });
  const baseUrl = resolveOverrideValue({
    ...
    envValue: hasScopedUser ? undefined : process.env.RXRESUME_URL,
