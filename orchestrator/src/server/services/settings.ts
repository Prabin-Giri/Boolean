...
export async function getEffectiveSettings(): Promise<AppSettings> {
  const registeredUsersExist = await hasUsers().catch((error) => {
    logger.warn("Failed to check whether login users exist", { error });
    return false;
  });
  ...
  const result: Partial<AppSettings> = {
    ...envSettings,
    authEnabled: Boolean(envSettings.basicAuthActive || registeredUsersExist),
  };
