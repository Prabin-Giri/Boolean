...
function getScopedDocumentId(id: string): string {
  const userId = getUserId();
  return userId ? `user:${userId}:${id}` : id;
}

export async function getLatestDesignResumeDocument() {
  const scopedId = getScopedDocumentId("primary");
  const [row] = await db
    .select()
    .from(designResumeDocuments)
    .where(eq(designResumeDocuments.id, scopedId))
...
export async function getDesignResumeDocumentById(id: string) {
  const scopedId = getScopedDocumentId(id);
  const [row] = await db
    .select()
    .from(designResumeDocuments)
    .where(eq(designResumeDocuments.id, scopedId))
...
export async function getDesignResumeAssetById(id: string) {
  const [row] = await db
    ...
    .innerJoin(
      ...
      eq(designResumeDocuments.id, designResumeAssets.documentId),
    )
    .where(
      and(
        eq(designResumeAssets.id, id),
        eq(designResumeDocuments.id, getScopedDocumentId("primary")),
      ),
    )
    .limit(1);
...
export async function upsertDesignResumeDocument(input: {
  ...
  if (existing) {
    ...
    await db.insert(designResumeDocuments).values({
      id: getScopedDocumentId(input.id),
      title: input.title,
      resumeJson: input.resumeJson,
      revision: input.revision,
      sourceResumeId: input.sourceResumeId,
