...
function getOwnedJobsFilter() {
  const userId = getUserId();
  return userId ? eq(jobs.userId, userId) : undefined;
}

function withOwnedJobsFilter(condition?: ReturnType<typeof eq>) {
  const ownerFilter = getOwnedJobsFilter();
  if (!ownerFilter) return condition;
  if (!condition) return ownerFilter;
  return and(ownerFilter, condition);
}
...
export async function getAllJobs(statuses?: JobStatus[]): Promise<Job[]> {
  const ownerFilter = getOwnedJobsFilter();
  const statusFilter =
    statuses && statuses.length > 0 ? inArray(jobs.status, statuses) : undefined;
  const query =
...
export async function getJobById(id: string): Promise<Job | null> {
  const [row] = await db
    .select()
    .from(jobs)
    .where(withOwnedJobsFilter(eq(jobs.id, id)));
  return row ? mapRowToJob(row) : null;
}

export async function listJobNotes(jobId: string): Promise<JobNote[]> {
  if (!(await getJobById(jobId))) return [];
  const rows = await db
    .select()
    .from(jobNotes)
...
export async function getJobNoteById(noteId: string): Promise<JobNote | null> {
  const [row] = await db
    .select({ note: jobNotes })
    .from(jobNotes)
    .innerJoin(jobs, eq(jobs.id, jobNotes.jobId))
    .where(withOwnedJobsFilter(eq(jobNotes.id, noteId)));
  return row ? mapRowToJobNote(row.note) : null;
}
...
export async function getJobNoteForJob(
  ...
  if (!(await getJobById(jobId))) return null;
  const [row] = await db
    .select()
    .from(jobNotes)
...
export async function createJobNote(
  ...
  if (!(await getJobById(input.jobId))) {
    throw new Error(`Job not found for note creation: ${input.jobId}`);
  }
  const id = randomUUID();
...
> {
  ...
  return db
    ...
    .from(jobs)
    .where(withOwnedJobsFilter(inArray(jobs.id, jobIds)));
}
...
export async function getJobByUrl(jobUrl: string): Promise<Job | null> {
  const [row] = await db
    .select()
    .from(jobs)
    .where(withOwnedJobsFilter(eq(jobs.jobUrl, jobUrl)));
  return row ? mapRowToJob(row) : null;
}
...
export async function getAllJobUrls(): Promise<string[]> {
  const ownerFilter = getOwnedJobsFilter();
  const query = db.select({ jobUrl: jobs.jobUrl }).from(jobs);
  const rows = ownerFilter ? await query.where(ownerFilter) : await query;
...
function isJobUrlUniqueViolation(error: unknown): boolean {
  ...
  return /UNIQUE constraint failed: jobs\.(job_url|user_id, jobs\.job_url)/i.test(
    error.message,
  );
}
...
export async function createJobs(
  ...
  const existingRows = await db
    ...
    .from(jobs)
    .where(withOwnedJobsFilter(inArray(jobs.jobUrl, uniqueUrls)));
...
export async function getJobStats(): Promise<Record<JobStatus, number>> {
  ...
  const query = db
    ...
    .from(jobs)
    .groupBy(jobs.status);
  const result = ownerFilter ? await query.where(ownerFilter) : await query;
...
export async function deleteJobsByStatus(status: JobStatus): Promise<number> {
  const result = await db
    .delete(jobs)
    .where(withOwnedJobsFilter(eq(jobs.status, status)))
    .run();
  return result.changes;
}
...
export async function deleteJobsBelowScore(threshold: number): Promise<number> {
  const result = await db
    ...
    .where(
      and(
        ...(getOwnedJobsFilter() ? [getOwnedJobsFilter()!] : []),
