...
import { hasUsers } from "@server/repositories/users";
...
export function createAuthGuard() {
  async function getAuthConfig() {
    ...
    const registeredUsersExist = await hasUsers().catch(() => false);
    return {
      user,
      pass,
      enabled: (user.length > 0 && pass.length > 0) || registeredUsersExist,
    };
  }
  ...
  const middleware = (
    ...
    void (async () => {
      ...
      const requiresAuthentication =
        enabled && requiresAuth(req.method, req.path);
      ...
      if (!requiresAuthentication) {
        if (token) {
          try {
            ...
            // Ignore invalid bearer tokens on public routes.
          }
        }
        next();
        return;
