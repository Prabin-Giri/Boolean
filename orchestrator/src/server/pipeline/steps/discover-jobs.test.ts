...
describe("discoverJobsStep", () => {
  ...
  it("streams filtered discovered jobs to the incremental import callback", async () => {
    const settingsRepo = await import("@server/repositories/settings");
    const registryModule = await import("@server/extractors/registry");
    const onDiscoveredJobs = vi.fn().mockResolvedValue({
      created: 1,
      skipped: 0,
    });
    ...
    vi.mocked(registryModule.getExtractorRegistry).mockResolvedValue({
      ...
      manifestBySource: new Map([["linkedin", manifest as any]]),
    ...
    expect(onDiscoveredJobs).toHaveBeenCalledWith([
      expect.objectContaining({
        employer: "Contoso",
        jobUrl: "https://example.com/job-2",
      }),
    ]);
    expect(result.imported).toEqual({ created: 1, skipped: 0 });
    expect(result.discoveredJobs).toHaveLength(1);
  });
