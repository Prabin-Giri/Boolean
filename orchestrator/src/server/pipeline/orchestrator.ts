...
export async function runPipeline(
  ...
  return runWithRequestContext({ pipelineRunId: pipelineRun.id }, async () => {
    ...
    try {
      ...
      ensureNotCancelled();
      const { discoveredJobs, imported } = await discoverJobsStep({
        mergedConfig,
        shouldCancel: () => cancelRequestedAt !== null,
        onDiscoveredJobs: async (jobs) => {
          return await jobsRepo.createJobs(jobs);
        },
      });
      ...
      const { created, skipped } =
        discoveredJobs.length > 0
          ? imported.created + imported.skipped > 0
            ? imported
            : await importJobsStep({ discoveredJobs })
          : { created: 0, skipped: 0 };
      if (imported.created + imported.skipped > 0) {
