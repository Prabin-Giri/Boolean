...
const migrations = [
  ...
  `CREATE INDEX IF NOT EXISTS idx_jobs_user_status_discovered_at ON jobs(user_id, status, discovered_at)`,
  `CREATE INDEX IF NOT EXISTS idx_jobs_discovered_at ON jobs(discovered_at)`,
  `CREATE INDEX IF NOT EXISTS idx_jobs_status_discovered_at ON jobs(status, discovered_at)`,
  `CREATE INDEX IF NOT EXISTS idx_pipeline_runs_user_started_at ON pipeline_runs(user_id, started_at)`,
  ...
  `PRAGMA foreign_keys = OFF`,
  `CREATE TABLE IF NOT EXISTS pipeline_runs_user_owned (
    id TEXT PRIMARY KEY,
    user_id TEXT NOT NULL DEFAULT '',
    started_at TEXT NOT NULL DEFAULT (datetime('now')),
    completed_at TEXT,
  ...
  `INSERT OR REPLACE INTO pipeline_runs_user_owned (
    id, user_id, started_at, completed_at, status, jobs_discovered, jobs_processed, error_message
  )
  SELECT
    id,
  ...
  FROM pipeline_runs`,
  `DROP TABLE IF EXISTS pipeline_runs`,
  `ALTER TABLE pipeline_runs_user_owned RENAME TO pipeline_runs`,
  `CREATE TABLE IF NOT EXISTS jobs_user_owned (
    id TEXT PRIMARY KEY,
    user_id TEXT NOT NULL DEFAULT '',
    source TEXT NOT NULL DEFAULT 'gradcracker',
    source_job_id TEXT,
  ...
  `INSERT OR REPLACE INTO jobs_user_owned (
    id, user_id, source, source_job_id, job_url_direct, date_posted, job_type, salary_source, salary_interval,
    salary_min_amount, salary_max_amount, salary_currency, is_remote, job_level, job_function, listing_type,
    emails, company_industry, company_logo, company_url_direct, company_addresses, company_num_employees,
    company_revenue, company_description, skills, experience_range, company_rating, company_reviews_count,
  ...
  SELECT
    id, COALESCE(NULLIF(user_id, ''), ''), source, source_job_id, job_url_direct, date_posted, job_type, salary_source, salary_interval,
    salary_min_amount, salary_max_amount, salary_currency, is_remote, job_level, job_function, listing_type,
    emails, company_industry, company_logo, company_url_direct, company_addresses, company_num_employees,
    company_revenue, company_description, skills, experience_range, company_rating, company_reviews_count,
    vacancy_count, work_from_home_type, title, employer, employer_url, job_url, application_link, disciplines,
  ...
  FROM jobs`,
  `DROP TABLE IF EXISTS jobs`,
  `ALTER TABLE jobs_user_owned RENAME TO jobs`,
  `PRAGMA foreign_keys = ON`,
  `CREATE INDEX IF NOT EXISTS idx_jobs_user_id ON jobs(user_id)`,
  `CREATE INDEX IF NOT EXISTS idx_jobs_user_status_discovered_at ON jobs(user_id, status, discovered_at)`,
  `CREATE INDEX IF NOT EXISTS idx_pipeline_runs_user_started_at ON pipeline_runs(user_id, started_at)`,
