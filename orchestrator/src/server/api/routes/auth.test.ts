...
describe.sequential("Auth routes", () => {
  ...
  describe("POST /api/auth/login", () => {
    ...
    it("accepts credentials for a registered user", async () => {
      await stopServer({ server, closeDb, tempDir });
      ({ server, baseUrl, closeDb, tempDir } = await startServer());
  ...
  describe("POST /api/auth/register", () => {
    ...
    it("creates a new account and returns a JWT", async () => {
      ...
      expect(body.ok).toBe(true);
      expect(body.data.token).toBeTruthy();
      expect(body.data.user.username).toBe("new-user");
    });
  ...
  describe("JWT-authenticated requests", () => {
    ...
    it("accepts a valid JWT from a registered user on protected routes", async () => {
      await stopServer({ server, closeDb, tempDir });
      ({ server, baseUrl, closeDb, tempDir } = await startServer());
