...
describe.sequential("Jobs API routes", () => {
  ...
  it("isolates the jobs workspace per authenticated user", async () => {
    const { createJob } = await import("@server/repositories/jobs");
    const { runWithRequestContext } = await import("@infra/request-context");

    const userA = await registerUser(baseUrl, "user-a-login");
    const userB = await registerUser(baseUrl, "user-b-login");
    ...
    await runWithRequestContext({ userId: userA.userId }, async () => {
      await createJob({
        ...
        title: "User A Role",
        employer: "Acme",
        jobUrl: "https://example.com/job/user-a",
        jobDescription: "Scoped to user A",
      });
    });
    ...
    await runWithRequestContext({ userId: userB.userId }, async () => {
      await createJob({
        ...
        title: "User B Role",
        employer: "Beta",
        jobUrl: "https://example.com/job/user-b",
        jobDescription: "Scoped to user B",
      });
    });

    const listA = await fetch(`${baseUrl}/api/jobs`, {
      headers: { Authorization: `Bearer ${userA.token}` },
    });
    const bodyA = await listA.json();
    expect(listA.status).toBe(200);
    expect(bodyA.data.total).toBe(1);
    expect(bodyA.data.jobs[0].title).toBe("User A Role");

    const listB = await fetch(`${baseUrl}/api/jobs`, {
      headers: { Authorization: `Bearer ${userB.token}` },
    });
    const bodyB = await listB.json();
    expect(listB.status).toBe(200);
    expect(bodyB.data.total).toBe(1);
    expect(bodyB.data.jobs[0].title).toBe("User B Role");
  });
