...
export function useOnboardingRequirement(enabled = true) {
  const { settings, isLoading: settingsLoading } = useSettings(enabled);
  const demoInfo = useDemoInfo(enabled);
  ...
  return {
    checking,
    complete: enabled ? complete : true,
  };
}
