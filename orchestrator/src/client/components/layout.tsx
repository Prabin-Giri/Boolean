...
export const PageHeader: React.FC<PageHeaderProps> = ({
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-border/80 bg-card/72 backdrop-blur-xl supports-[backdrop-filter]:bg-card/68">
      <div className="container mx-auto flex flex-col gap-3 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
        <div className="flex min-w-0 items-center gap-3">
          <Sheet open={navOpen} onOpenChange={setNavOpen}>
            ...
            <SheetContent
              side="left"
              className="flex w-64 flex-col border-r border-border/80 bg-card/96"
            >
              <SheetHeader>
                <SheetTitle className="font-headline text-lg tracking-tight">
                  JobOps
                </SheetTitle>
              </SheetHeader>
              <nav className="mt-6 flex flex-col gap-2">
                {NAV_LINKS.map(({ to, label, icon: NavIcon, activePaths }) => (
                  <button
                    ...
                    className={cn(
                      "flex items-center gap-3 rounded-2xl px-3.5 py-2.5 text-left text-sm font-medium transition-colors hover:bg-accent/85 hover:text-accent-foreground",
          ...
          <div className="flex h-10 w-10 items-center justify-center rounded-2xl border border-border/80 bg-primary/8 shadow-xs">
            <Icon className="h-4 w-4 text-primary" />
          </div>
          <div className="min-w-0 leading-tight">
            <div className="font-headline text-base font-semibold tracking-tight">
              {title}
            </div>
...
export const ListPanel: React.FC<ListPanelProps> = ({
  ...
  <div
    className={cn(
      "min-w-0 flex flex-col rounded-[28px] border border-border/80 bg-card/88 shadow-lg backdrop-blur-sm",
      className,
    )}
  >
    {header && (
