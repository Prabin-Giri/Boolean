...
describe("API client auth flow", () => {
  ...
  it("stores a bearer token when registering directly", async () => {
    vi.spyOn(global, "fetch").mockResolvedValueOnce(
      jwtLoginSuccess("registered-token"),
    );

    await expect(
      api.registerWithCredentials("new-user", "super-secret"),
    ).resolves.toBeUndefined();
    expect(api.getCachedAuthHeader()).toBe("Bearer registered-token");
  });

  it("clears the cached token on logout", async () => {
