...
describe("OnboardingPage", () => {
  ...
  it("defaults the authentication step to lock it down", async () => {
    ...
    expect(
      screen.getByRole("button", { name: /ready and apply/i }),
    ).toBeInTheDocument();
  });
  ...
  it("skips onboarding entirely when login auth is already enabled", async () => {
    currentSettings = {
      ...baseSettings,
      authEnabled: true,
    };
    ...
    expect(screen.queryByText("Secure your workspace")).not.toBeInTheDocument();
  });

  it("waits for the shared onboarding check before redirecting to ready", async () => {
    currentSettings = {
      ...baseSettings,
      authEnabled: true,
    };
    ...
    expect(await screen.findByText("Finishing onboarding...")).toBeInTheDocument();
    expect(screen.queryByText("ready page")).not.toBeInTheDocument();
  });

  it("advances to the next step after saving the LLM connection", async () => {
    vi.mocked(api.validateLlm).mockResolvedValue({
      valid: true,
      message: null,
    });
    ...
    vi.mocked(api.updateSettings).mockImplementation(async () => currentSettings);
  ...
  it("lets the user skip basic auth and go straight to ready from the last step", async () => {
    vi.mocked(api.validateLlm).mockResolvedValue({
      valid: true,
      message: null,
    });
    ...
    fireEvent.click(screen.getByRole("button", { name: /ready and apply/i }));
  ...
  it("advances after saving the LLM step", async () => {
    vi.mocked(api.validateLlm).mockResolvedValue({
      valid: true,
      message: null,
    });
    ...
    expect(
      screen.queryByText("Choose the LLM connection Job Ops should use."),
    ).not.toBeInTheDocument();
  });
