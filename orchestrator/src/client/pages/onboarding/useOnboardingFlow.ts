...
export function useOnboardingFlow() {
  ...
  const canExitToReadyFromLastStep =
    currentStep === "basicauth" &&
    steps
      .filter((step) => step.id !== "basicauth")
  ...
  const handlePrimaryAction = useCallback(async () => {
    ...
    if (currentStep === "llm") {
      const success = await handleSaveLlm();
      if (success) {
        advanceToNextStep();
    ...
    if (currentStep === "baseresume") {
      if (resumeSetupMode === "rxresume") {
        const success = await handleSaveRxresume();
        if (success) {
          advanceToNextStep();
          return "advanced" as const;
        }
      ...
      const success = await handleSaveBaseResume();
      if (success) {
        advanceToNextStep();
        return "advanced" as const;
      }
    ...
    if (currentStep === "searchterms") {
      const success = await handleSaveSearchTerms();
      if (success) {
        advanceToNextStep();
        return "advanced" as const;
      }
    ...
    const success = await handleCompleteBasicAuth();
    if (success && canExitToReadyFromLastStep) {
